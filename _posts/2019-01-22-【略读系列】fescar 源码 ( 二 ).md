...
上一章总结我们把主线给走完了，具体 RM 和 TC 如何通信，RM 分支事务注册，提交这些细节，我们还没走读。现在我们就来走读一下这些代码。
...
这里会触发 RM 和 TC 的通信。我们可以看到在配置文件中有数据源代理的配置。
...
这里我们知识走完的分支事务的流程，也就是 RM 的一些代码走读，关于分支事务注册，以及分支事务状态上报，我们后天继续。

# 后续补充
1. xid的传递
2. 全局事务写锁的原理
3. AutoCommit 中 false 和 true 的影响
